...
photo = 'car_person.jpg'

with open(photo, 'rb') as image_file:
    source_bytes = image_file.read()

detect_file = client.detect_labels(Image = {'Bytes': source_bytes})
# print(detect_file)

image = Image.open(io.BytesIO(source_bytes))
draw = ImageDraw.Draw(image)

for label in detect_file['Labels']:
    print(label["Name"])
    ...
    for instances in label['Instances']:
        ...
        if 'BoundingBox' in instances:
            ...
            fort = ImageFont.truetype("arial.ttf", 30)
            
            draw.text((left + 10, top - 30), label["Name"], fort=fort, fill= "#000000")
